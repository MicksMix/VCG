...
{
	sealed class modVBCheck
	{
		
		// Specific checks for VB code
		//============================
		
		public static void CheckVBCode(string CodeLine, string FileName)
		{
			...
			modCSharpCheck.CheckInputValidation(CodeLine, FileName); //(same check will work for VB and C# - hence we use function in C# module)
			modPHPCheck.CheckSQLInjection(CodeLine, FileName); // Check for SQLi (same check will work for VB and C# - hence we use function in C# module)
			modPHPCheck.CheckXSS(CodeLine, FileName); // Check for XSS (same check will work for VB and C# - hence we use function in C# module)
			modCSharpCheck.CheckSecureStorage(CodeLine, FileName); // Are sensitive variables stored without using SecureString? (same check will work for VB and C# - hence we use function in C# module)
			modPHPCheck.CheckLogDisplay(CodeLine, FileName); // Is data sanitised before being written to logs? (same check will work for VB and C# - hence we use function in C# module)
			modCSharpCheck.CheckFileRace(CodeLine, FileName); // Check for race conditions and TOCTOU vulns (same check will work for VB and C# - hence we use function in C# module)
			modCSharpCheck.CheckHTTPRedirect(CodeLine, FileName); // Check for safe redirects and safe use of URLs (same check will work for VB and C# - hence we use function in C# module)
			...
			modPHPCheck.CheckExecutable(CodeLine, FileName); // Check for unvalidated variables being executed via cmd line/system calls (same check will work for VB and C# - hence we use function in C# module)
			modCSharpCheck.CheckWebConfig(CodeLine, FileName); // Check config file to determine whether .NET debugging and default errors are enabled
		...
		private static void CheckRandomisation(string CodeLine, string FileName)
		{
			...
			if (Regex.IsMatch(CodeLine, "\\bRnd\\b\\s*\\("))
			{
		...
		private static void CheckSAML2Validation(string CodeLine, string FileName)
		{
			...
			if (modMain.ctCodeTracker.IsSamlFunction == false && Regex.IsMatch(CodeLine, "\\bOverrides\\b\\s+\\b(Sub|Function)\\b\\s+\\bValidateConditions\\b\\(\\bSaml2Conditions\\b"))
			{
			...
			else if (modMain.ctCodeTracker.IsSamlFunction == true)
			{
				...
				if (Regex.IsMatch(CodeLine, "\\bEnd\\b\\s+\\b(Sub|Function)\\b"))
				{
					modMain.ctCodeTracker.IsSamlFunction = true;
				}
			}
		...
		private static void CheckUnsafeTempFiles(string CodeLine, string FileName)
		{
			...
			if (Regex.IsMatch(CodeLine, "(file\\S*|File\\S*|\\.FileName)\\s+\\=\\s+\\\"\\S*(temp|tmp)\\S*\\\"\\,"))
			{
		...
		private static void CheckCryptoKeys(string CodeLine, string FileName)
		{
			// Identify any hardcoded static keys and IVs
			//===========================================
			
			if (Regex.IsMatch(CodeLine, "\\b(Private|Public|Dim)\\b\\s+\\b(Const|ReadOnly)\\b\\s+\\w*(crypt|Crypt|CRYPT|key|Key|KEY)\\w*\\s+As\\s+String\\s*\\=\\s*\\\"") || Regex.IsMatch(CodeLine, "\\b(Private|Public|Dim)\\b\\s+\\b(Const|ReadOnly)\\b\\s+\\w*(iv|Iv|IV)\\s+As\\s+Byte\\(\\)\\s*\\=\\s*New\\s+Byte\\s*\\(\\w*\\)\\s*\\{"))
			{
				frmMain.Default.ListCodeIssue("Hardcoded Crypto Key", "The code appears to use hardcoded encryption keys. These can be rendered visible with the use of debugger or hex editor, exposing encrypted data.", FileName, CodeIssue.MEDIUM, CodeLine);
			}
